...
#define PWM_MAX_2_CW      60//50        //Max value for the joint 2 PWM when needs to go CW.
#define PWM_MIN_2_CW      0         //Min value for the joint 2 PWM when needs to go CW.

#define PWM_MAX_2_CCW     110//150       //Max value for the joint 2 PWM when needs to go CCW (against gravity).
#define PWM_MIN_2_CCW     80//80        //Min value for the joint 2 PWM when needs to go CCW (against gravity).
...
void GoHome(){
  ...
  while(!LS_1Bstat){
    //Forces small changes in PWM.
    if(reset_output_1 < PWM_MAX_1*1.3)
      reset_output_1 = reset_output_1 + 1;
    else
      reset_output_1 = PWM_MAX_1*1.3;
